...
changeLocationQuantityRouter.post("/", async (req, res) => {
  ...
  try {
    ...
    if (old_quantity - moveAmount < 0) {
      ...
      // query thatgrabs the current quantity from the new location the items will be moved to
      let getNewLocationQtyQuery = `select quantity from part_quantity WHERE internal_part_number = '${internal_part_number}' AND status_id = ${new_status_id} AND location_id = ${new_location_id};`;
      const newLocationCurQty = await pool.query(getNewLocationQtyQuery);

      // gets the updated quantity of the new location after moving items there
      let newLocationQty = newLocationCurQty[0].quantity + moveAmount;
      ...
      let updateNewLocationQuery = `UPDATE part_quantity SET quantity =${newLocationQty} WHERE internal_part_number = '${internal_part_number}' AND status_id = ${new_status_id} AND location_id = ${new_location_id};`;
      //query that creates a row for where the parts will be moved into if the row doesn't already exist
      let insertIntoNewLocationQuery = `insert into part_quantity values ('${internal_part_number}','${new_location_id}','${new_status_id}',${moveAmount},' ' );`;

      if (newLocationCurQty.length == 0) {
